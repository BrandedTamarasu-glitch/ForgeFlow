...
const commandInterfaceLearning = helperWrapper('commands/forgeflow-command-interface-learning.md', 'command-interface-learning.js');
const commandInterfaceObserve = helperWrapper('commands/forgeflow-command-interface-observe.md', 'record-command-interface-observation.js');
...
const checks = [
  ...
  ['command interface evidence wrapper supports codex fallback and safe parsing', commandInterfaceEvidence.referencesHelper && commandInterfaceEvidence.checksRepoLocalFile && commandInterfaceEvidence.fallsBackToInstalledHelper && commandInterfaceEvidence.scrubsNodeEnvironment && commandInterfaceEvidence.invokesHelperWithSafeArgs && commandInterfaceEvidence.markdown.includes('CODEX_HOME="${CODEX_HOME:-$HOME/.codex}"') && commandInterfaceEvidence.markdown.includes('ARG_HELPER="${ROOT}/scripts/forgeflow/command-args.js"') && commandInterfaceEvidence.markdown.includes('--allow "--input:path,--project-dir:path,--write-report:boolean,--json:boolean"') && commandInterfaceEvidence.markdown.includes("while IFS= read -r -d $'\\0' arg; do") && commandInterfaceEvidence.markdown.includes('Missing required --input') && commandInterfaceEvidence.markdown.includes('--write-report requires --project-dir') && !commandInterfaceEvidence.markdown.includes('read -r -a USER_ARGS')],
  ['command interface learning wrapper supports codex fallback and safe parsing', commandInterfaceLearning.referencesHelper && commandInterfaceLearning.checksRepoLocalFile && commandInterfaceLearning.fallsBackToInstalledHelper && commandInterfaceLearning.scrubsNodeEnvironment && commandInterfaceLearning.invokesHelperWithSafeArgs && commandInterfaceLearning.markdown.includes('CODEX_HOME="${CODEX_HOME:-$HOME/.codex}"') && commandInterfaceLearning.markdown.includes('ARG_HELPER="${ROOT}/scripts/forgeflow/command-args.js"') && commandInterfaceLearning.markdown.includes('--allow "--input:path,--candidate-id:value,--set-suggestions:value,--write:boolean,--write-policy:boolean,--json:boolean"') && commandInterfaceLearning.markdown.includes("while IFS= read -r -d $'\\0' arg; do") && !commandInterfaceLearning.markdown.includes('mapfile -d')],
  ['command interface observe wrapper supports codex fallback and safe parsing', commandInterfaceObserve.referencesHelper && commandInterfaceObserve.checksRepoLocalFile && commandInterfaceObserve.fallsBackToInstalledHelper && commandInterfaceObserve.scrubsNodeEnvironment && commandInterfaceObserve.invokesHelperWithSafeArgs && commandInterfaceObserve.markdown.includes('CODEX_HOME="${CODEX_HOME:-$HOME/.codex}"') && commandInterfaceObserve.markdown.includes('ARG_HELPER="${ROOT}/scripts/forgeflow/command-args.js"') && commandInterfaceObserve.markdown.includes('--allow "--work-item-id:value,--command-id:value,--outcome:value,--command-calls:value,--decision-output-bytes:value,--json:boolean"') && commandInterfaceObserve.markdown.includes("while IFS= read -r -d $'\\0' arg; do") && !commandInterfaceObserve.markdown.includes('mapfile -d')],
