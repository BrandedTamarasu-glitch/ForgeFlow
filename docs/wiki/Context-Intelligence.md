...
| Command capability matrix | `scripts/forgeflow/render-command-capability-matrix.js` | Renders a read-only matrix of command wrapper, Pi alias, OpenCode command, and skill coverage so host surface gaps are visible without launching host applications. |
...
| Lean host CLI probes | `scripts/forgeflow/render-lean-host-cli-probes.js` | Checks PATH for optional host CLI executable names and prints manual probe commands without running host CLIs. |
...
| Lean benchmark runner | `scripts/forgeflow/render-lean-benchmark-runner.js` | Renders an opt-in benchmark runner scaffold with task arms, aggregate comparison commands, and explicit model-runner placeholders. Default output never calls models or the network. |
...
| Stale artifact plan | `scripts/forgeflow/render-stale-artifact-plan.js` | Converts stale trend, latest-insights, failure-digest, and context-budget signals into minimal refresh commands without refreshing or deleting local artifacts. |
