...
- **Lean decision, mode, review, benchmark, and metrics:** `/forgeflow-lean-decision --task "<work item>"` gives a read-only minimum-sufficient-solution check before implementation. It points to native, standard-library, installed-dependency, or project-pattern reuse candidates, lists avoid-first work, names do-not-simplify boundaries such as security, accessibility, trust-boundary validation, data-loss prevention, and explicit requirements, then gives the focused validation minimum plus the known ceiling and upgrade trigger for expanding later. `/forgeflow-lean-mode --profile off|balanced|strict|ultra --write` persists a project-local advisory lean profile in `.forgeflow/<project>/context/lean-policy.{md,json}`; `off` blocks lean context injection, while `balanced`, `strict`, and `ultra` tune the compact guidance budget without changing implementation, review, commit, push, or network behavior. Optional inline markers such as `forgeflow: lean: <reason>`, `forgeflow: upgrade when: <trigger>`, `forgeflow: no-new-deps`, `forgeflow: stdlib-first: <reason>`, `forgeflow: native-first: <reason>`, and `forgeflow: reuse-first: <reason>` act as advisory breadcrumbs for why a small path was chosen. `/consult`, `/implement`, Codex consult/implement skills, and generated project-intelligence brief stubs carry the same compact lean section into handoffs when the helper is available. `/implement` also writes a lean-decision JSON sidecar so Atlas can record the ceiling and upgrade trigger in implementation notes with `record-implementation-notes.js --lean-decision`. `/forgeflow-lean-review` adds a separate read-only over-engineering lane for `delete`, `stdlib`, `native`, `reuse`, `yagni`, `shrink`, and `prose-bloat` findings, enriched with static project evidence from code topology, invocation hints, package dependency deltas, and lean markers when those artifacts exist. `/forgeflow-lean-benchmark --baseline <json> --current <json>` compares local aggregate baseline and lean-guided metrics for files, lines, validation signals, review findings, prose warnings, ceiling captures, follow-up signals, and context-token savings; `--write` stores `.forgeflow/<project>/context/lean-benchmark.{md,json}`. `/forgeflow-output-contract --lean-file <path>` can warn when a generated lean handoff or review note has too much narrative, while preserving raw command output, diffs, failure evidence, review evidence, and user-requested explanations. `/forgeflow-lean-report` summarizes local aggregate lean signals such as files and lines touched, reuse and avoid-first counts, validation and ceiling capture, review/prose warnings, context-token savings, and telemetry quality without raw snippets or hosted telemetry. Context packs inject the compact lean block only when lean mode permits it and lean-decision, lean-report, latest-insights, profile, operating-model, and telemetry gates pass. It is not a correctness, security, accessibility, performance, or validation review and never applies fixes or workflow policy changes.
