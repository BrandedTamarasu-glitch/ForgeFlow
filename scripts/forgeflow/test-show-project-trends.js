...
const checks = [
  ...
  ['summarizes advisor', result.advisor.budget_status === 'fail' && result.advisor.code_map_trends_status === 'attention' && result.advisor.recommendations.some((item) => item.action === 'trim-budget-violation' && item.evidence && item.clears && item.split_suggestion && item.split_suggestion.strategy === 'split-before-review')],
