...
| `/forgeflow-command-interface-learning` | Preview a qualifying aggregate command-interface pattern as advisory project memory. Suggestions are on by default and can be suppressed; durable memory requires the exact preview ID and `--write`. |
| `/forgeflow-command-interface-observe` | Record one local sanitized command observation using only normalized IDs, an outcome, and compact counts. It never receives shell text or raw output and does not promote memory. |
...
| `/forgeflow-memory-correct` | Preview an exact project-memory correction by default. Add `--write` to append a replacement learning and retirement event for one active ID, without deleting or rewriting history. It never reads or changes user-profile memory. |
...
| `/forgeflow-lean-benchmark-results` | Validate aggregate benchmark result metadata, task and arm coverage, correctness gates, cost/latency metrics, session-cost caveats, evidence grade, and per-arm summaries before publishing lean performance claims. |
| `/forgeflow-lean-benchmark-runner` | Render an opt-in benchmark runner scaffold with task arms, historical replay tasks, evidence checklist, stored-evidence summary, and explicit model-runner commands; default output never calls models or the network. `--run` executes Promptfoo only when `FORGEFLOW_BENCHMARK_ALLOW_NETWORK=1` and a local promptfoo executable is available. |
...
| `/forgeflow-lean-host-cli-probes` | Inspect PATH for optional host CLI executables and print manual lean adapter probe commands without launching host applications. Add `--write-template` to create a local proof template or `--evidence <json>` to mark manually verified probes with timestamp, note, and optional output digest. |
...
| `/forgeflow-lean-prime` | Show a single first-run checklist for lean mode, decision evidence, report evidence, telemetry quality, and context-injection eligibility, with one next command and a bootstrap command to clear blocked work-item evidence. Add `--prime-task <text>` to write the lean decision and prime plan artifacts for the current work item. |
...
| `/forgeflow-skills` | Check or regenerate committed core `skills/forgeflow-*/SKILL.md` packages for skill-capable hosts. |
...
| `/forgeflow-release-readiness` | Run advisory local release readiness checks, verify runtime helper sources are present, managed, regular files, and inside the checkout, group blockers by readiness area, and avoid tagging, pushing, publishing, or GitHub calls. It also reports advisory evidence warnings for benchmark proof, host probe verification, failure-digest aftercare, and RTK command-policy alignment. The helper supports `--baseline <json>`, `--compare-last`, and `--save-current` for prior-run comparison. |
...
| `/forgeflow-stale-artifact-plan` | Show minimal slash-command refresh steps and build-aftercare guidance for stale local guidance artifacts without refreshing or deleting them. |
