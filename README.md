...
- **Lean decision contract:** `/forgeflow-lean-decision --task "<work item>"` gives a read-only minimum-sufficient-solution check before implementation. It points to native, standard-library, installed-dependency, or project-pattern reuse candidates, lists avoid-first work, names do-not-simplify boundaries such as security, accessibility, trust-boundary validation, data-loss prevention, and explicit requirements, then gives the focused validation minimum plus the known ceiling and upgrade trigger for expanding later.
