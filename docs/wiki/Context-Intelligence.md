...
| Runtime inventory | `scripts/forgeflow/runtime-inventory.js` | Provides shared command and runtime-helper inventory reads for coverage tests, health inventory checks, helper owner-surface grouping, command/helper counts, installed helper names, and install/update/release/docs registry consolidation. |
...
| Outcome capture plan | `scripts/forgeflow/render-outcome-capture-plan.js` | Shows which local outcome evidence streams are missing and prints recorder prompts for next-work outcomes, review outcomes, and agent feedback without writing records. |
...
| Validation failure capture | `scripts/forgeflow/render-validation-failure-capture.js` | Maps a failed validation command to the safest capture mode and failure-digest path without executing commands; correctness-critical output stays raw-required. |
