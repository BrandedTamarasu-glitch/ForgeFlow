...
const files = {
  ...
  smith: read('agents/builder-implement.md'),
  warden: read('agents/guardian-implement.md'),
  lumen: read('agents/designer-implement.md'),
  compass: read('agents/product-lead-implement.md'),
  atlas: read('agents/coordinator-implement.md'),
  arbiterImplement: read('agents/architect-implement.md'),
  arbiterConsult: read('agents/architect-consult.md'),
  atlasPresent: read('agents/coordinator-present.md'),
...
const checks = [
  ...
  ['ship prepare excludes unscoped log entries', preparedSummary.implementation_notes?.decisions?.length === 0],
  ['ship prepare labels historical context without importing it', preparedHtml.includes('Historical context only') && !preparedHtml.includes('Ship helper needed a summary bridge')],
  ['ship prepare writes notes check', preparedCheck.status === 'pass' && preparedCheck.ship_summary === preparedSummaryPath],
  ['ship prepare body excludes local notes check', !preparedBody.includes('implementation-notes-check.json')],
  ...
  ['ship prepare body excludes local lean report', !preparedBody.includes('lean-readiness.json')],
  ['ship prepare refreshes project learnings', preparedLearnings.out === path.join(shipNotesDir, 'project-learnings.md')],
  ['ship prepare body excludes local learnings', !preparedBody.includes('project-learnings-rollup.json') && !preparedBody.includes('.forgeflow/')],
