...
const checks = [
  ...
  ['rtk session guidance included without failing health', fixed.checks.some((item) => item.name === 'rtk command wrapper on PATH' && ['pass', 'warn', 'skip'].includes(item.status)) && renderMarkdown(fixed).includes('rtk command wrapper on PATH')],
  ['rtk warning renders repair guidance when missing', !fixed.checks.find((item) => item.name === 'rtk command wrapper on PATH' && item.status === 'warn') || renderMarkdown(fixed).includes('verify: rtk --version && rtk gain')],
  ['rtk warning records policy alignment impact', !fixed.checks.find((item) => item.name === 'rtk command wrapper on PATH' && item.status === 'warn') || fixed.checks.find((item) => item.name === 'rtk command wrapper on PATH').policy_alignment === 'optional-command-wrapper'],
