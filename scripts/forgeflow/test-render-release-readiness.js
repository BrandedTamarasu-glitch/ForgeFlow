...
const checks = [
  ...
  ['evidence advisory warnings are included', result.categories['evidence-readiness'].total === 4 && result.categories['evidence-readiness'].warnings >= 3 && result.checks.some((item) => item.command === 'rtk policy alignment advisory')],
  ...
  ['post-publish readiness renders verification', postPublishResult.post_publish_verification && postPublishResult.post_publish_verification.status === 'published-propagation-pending' && postPublishResult.post_publish_verification.summary.attention.length > 0 && postPublishMarkdown.includes('## Post-Publish Verification') && postPublishMarkdown.includes('published-propagation-pending') && postPublishMarkdown.includes('Passed evidence:') && postPublishMarkdown.includes('Attention evidence:')],
  ...
  ['markdown renders blockers', markdown.includes('# Forgeflow Release Readiness') && markdown.includes('install helper missing') && markdown.includes('Kind: command-failure') && markdown.includes('## Baseline Comparison') && markdown.includes('No baseline compared: no baseline provided') && markdown.includes('## Release To Install Preflight') && markdown.includes('present,') && markdown.includes('## Advisory Warnings') && (markdown.includes('rtk policy alignment advisory') || result.checks.some((item) => item.command === 'rtk policy alignment advisory' && item.status === 'pass')) && markdown.includes('Release readiness is advisory') && markdown.includes('unless --save-current is passed')],
