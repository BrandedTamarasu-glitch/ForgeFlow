...
| Project trends | `scripts/forgeflow/show-project-trends.js` | Summarizes code-map trend status, artifact freshness, latest-insights readiness/freshness, project-learning consumption, and advisor health from existing local artifacts, with optional `--refresh` first. |
