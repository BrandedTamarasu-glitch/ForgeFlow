...
fs.writeFileSync(path.join(projectDir, 'project-learning-candidates.jsonl'), [
  ...
  JSON.stringify({
    schema_version: '1',
    category: 'stable-decision',
    learning: 'Use canary deploys for this release path',
    source: 'Atlas',
    confidence: 'high',
    evidence_count: 2,
    conflict_key: 'release_channel',
    conflict_value: 'canary',
  }),
  JSON.stringify({
    schema_version: '1',
    category: 'stable-decision',
    learning: 'Use direct deploys for this release path',
    source: 'Compass',
    confidence: 'high',
    evidence_count: 2,
    conflict_key: 'release_channel',
    conflict_value: 'direct',
  }),
  '',
].join('\n'));
...
const checks = [
  ...
  ['omits explicitly conflicting candidates from guidance', !rendered.includes('Use canary deploys for this release path') && !rendered.includes('Use direct deploys for this release path') && !JSON.stringify(result).includes('release_channel')],
