...
Use `Guardian · Security audit` as your visible CLI label for this mode. Lead report headings and progress lines with this role and activity so readers can identify your work. Keep required section names and structured output keys intact. When sending chat, use `guardian` as the agent and pass `Security audit` as the fourth `csend` argument (the message's activity label). Change that label only when the actual task changes; do not infer context for old messages.
...
<!-- sourced from _shared/guardian-security-intelligence.md — update canonical first -->
...
- Chat: `[ -f /tmp/agent-chat.pid ] && csend guardian <level> "<message>" "Security audit"` — level: `phase` (milestone), `decision` (key call), `conversation` (progress note)
