...
| Learning status | `scripts/forgeflow/show-learning-status.js` | Summarizes project learnings, user profile, agent feedback, review outcomes, next-work outcomes, and first-run results into one advisory health view with fix-first, watch, healthy lanes, and signal-quality scoring with age/reinforcement decay. |
...
| Review-auto classifier | `scripts/forgeflow/classify-review-auto.js` | Previews safe, risky, and blocker buckets from captured review findings JSON before `/review-auto` applies any conservative fixes. |
