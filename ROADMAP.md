...
**Next item: F3.4 — Provider and release pilot.** Prepare bounded task-based comparisons with independently challenged controls, separate answer keys and explicit platform/accessibility limits. F3.3 now verifies installed Linux fixture identity, process launches and profile lifecycle, with simulated installation clearly separated. All six implemented capabilities remain in evaluation; no production release or model benefit is claimed.
...
**Session checkpoint:** implementation is complete through F3.3; F3.4 has not started. See [current status and resumption steps](STATUS.md). Phase 1 was published through `cebf945`; the packaged release version is unchanged.
