...
**Current follow-up: prepare broader real-project retests.** The twelve-review comparison is complete: each arm found three distinct confirmed defects, with one unique to each arm. The enhanced-only scenario defect supplies case-specific added value; the baseline-only malformed-backup defect prevents a net superiority claim. Neither exclusive finding repeated. Prepare the requested retest across all four earlier capabilities with separate treatments; keep normal activation gated. See [method and results](docs/real-project-review-results.md). F4.1 benchmark verification remains the next implementation item. All six implemented capabilities remain in evaluation; no production release or general model-benefit claim is established.
...
**Session checkpoint:** complete through F3.4 reporting; the real-project comparison is complete and no trials are running. Broader retest preparation is next; F4.1 remains the next implementation item. See [current status and resumption steps](STATUS.md). Phase 1 was published through `cebf945`; the packaged release version is unchanged.
...
### Real-project review comparison (2026-09-23)

Twelve fresh-context reviews completed on two submitted PR revisions and one application commit, with existing guidance versus added procedures. Four distinct defects were independently reproduced across both arms: two shared, one enhanced-only, one baseline-only. Each arm found three; exclusive findings occurred in one of two repetitions. Two other reproduced mechanisms remain policy/contract questions. No source mutations or recorded budget failures. The enhanced-only scenario issue and baseline-only malformed-backup issue are absent from the saved precommit audit. This is complementary case-specific discovery, not a net or replicated accuracy gain. Prepare broader retests of all four earlier capabilities, preserving original results and holding normal activation. F6.1 remains unchecked; this two-repeat exploratory study does not complete that milestone. See [results and limits](docs/real-project-review-results.md).
