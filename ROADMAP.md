...
**Current item: F3.4 — Provider and release pilot, preparation validated.** Candidate repository tasks, control variants, hidden executable checks, treatment snapshots and a 36-slot baseline/checklist/full schedule are prepared. Independent control review, final freeze and authorized model execution remain pending. F3.3 now verifies installed Linux fixture identity, process launches and profile lifecycle, with simulated installation clearly separated. All six implemented capabilities remain in evaluation; no production release or model benefit is claimed.
...
**Session checkpoint:** implementation is complete through F3.3; F3.4 preparation is validated, with no new model trials or independent control reviews yet. See [current status and resumption steps](STATUS.md). Phase 1 was published through `cebf945`; the packaged release version is unchanged.
