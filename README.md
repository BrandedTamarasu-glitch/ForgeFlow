...
- **Support bundle:** `/forgeflow-support` writes a local support/debug bundle with version, health, smoke, plan-only release readiness, docs drift, project trends, and consolidated next actions under `.forgeflow/<project>/support/`. It may include local paths, so review and redact it before sharing outside the trusted project/team context.
