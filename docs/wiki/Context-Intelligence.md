...
| Lean mode | `scripts/forgeflow/render-lean-mode.js` | Shows or persists the project-local lean guidance profile. Profiles are `off`, `lite`, `balanced`, `strict`, and `ultra`; `--write` saves `.forgeflow/<project>/context/lean-policy.{md,json}`. Context packs default to balanced or `FORGEFLOW_LEAN_DEFAULT_MODE` when no policy exists, block lean guidance when the policy is off, and include the selected profile when lean guidance is injected. |
| Lean portability pack | `scripts/forgeflow/render-lean-portability-pack.js` | Generates or checks portable lean rule copies for generic agent hosts, Cursor, Windsurf, Copilot, and skill-style adapters. It is read-only by default and writes only `.forgeflow/<project>/lean-portability/` with `--write`. |
