...
| Release follow-through | `scripts/forgeflow/render-release-follow-through.js` | Summarizes post-publish release verify, update verify, and runtime-consumability follow-through without tagging, pushing, publishing, repairing installs, or calling GitHub. |
...
| Learning status | `scripts/forgeflow/show-learning-status.js`, `scripts/forgeflow/learning-signal-policy.js` | Summarizes project learnings, user profile, agent feedback, review outcomes, next-work outcomes, and first-run results into one advisory health view with fix-first, watch, healthy lanes, signal-quality scoring with configurable age/reinforcement decay, exact outcome-capture commands, and event-specific workflow-ending capture prompts for missing calibration streams. The policy helper can compare a proposed JSON policy without writing it. |
| First useful win | `scripts/forgeflow/render-first-useful-win.js` | Summarizes early public-safe value evidence from first-run results, pilot evidence, agent feedback, and learning status so a new user can see what Forgeflow has already helped with. |
