...
| Runtime inventory | `scripts/forgeflow/runtime-inventory.js` | Provides shared command and runtime-helper inventory reads for coverage tests, health inventory checks, and future install/update registry consolidation. |
...
| Learning status | `scripts/forgeflow/show-learning-status.js`, `scripts/forgeflow/learning-signal-policy.js` | Summarizes project learnings, user profile, agent feedback, review outcomes, next-work outcomes, and first-run results into one advisory health view with fix-first, watch, healthy lanes, and signal-quality scoring with configurable age/reinforcement decay. The policy helper can compare a proposed JSON policy without writing it. |
...
| Validation plan | `scripts/forgeflow/render-validation-plan.js` | Maps changed files to focused validation commands and flags when full suite or source smoke are required. |
