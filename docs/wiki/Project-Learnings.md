...
For an explicit disagreement, project-learning candidates may use a paired `conflict_key` and `conflict_value`. Forgeflow withholds every active candidate in a key with two or more distinct values, then reports aggregate counts only. It never infers conflicts from prose; correct or clarify the records before relying on that guidance.
