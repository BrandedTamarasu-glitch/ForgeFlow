...
| `/forgeflow-lean-review` | Show a separate read-only over-engineering review lane for current diff or `--diff <path>`, using only `delete`, `stdlib`, `native`, `reuse`, `yagni`, `shrink`, and `prose-bloat` tags. Findings include static project evidence from topology, invocation hints, dependency deltas, and optional `forgeflow: lean` markers when available. Clean diffs end with `Lean already. Ship.` |
