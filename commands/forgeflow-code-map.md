...
- **Accepted gaps are local-only.** Add exact `{source, specifier}` or `{source, expression}` entries to `.forgeflow/<project>/code-map-accept.json` only for known intentional gaps. Add a maintainer `note`, and optionally date-only `accepted_at` / `expires_at` values in `YYYY-MM-DD` form, so the acceptance can be reviewed later. Accepted gaps stay visible, stale or expired acceptances are reported, and acceptance is not proof of runtime correctness.
