...
- **Review wave prep and outcome capture:** `/forgeflow-context-wave-plan` now flags incomplete zero-file packets, prioritizes first waves using path risk, topology hubs, changed-neighborhood hints, and proof-file markers, and still writes files only with `--write-wave-files`. `/forgeflow-review-wave-prep` turns the plan into the first focused review-wave command without spawning reviewers. `/forgeflow-outcome-capture-plan` shows which local outcome streams still need real recorder evidence so future next-work ranking can calibrate against usefulness instead of guesswork.
