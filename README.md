...
- **Safe command-output reduction:** `compact-command-output.js`, `capture-command-output.js`, `build-failure-digest.js`, and `advise-noisy-command.js` compact only allowlisted human-narrative output, preserve raw output for correctness-critical commands, stamp failure digests with Git provenance, advise narrower invocations before large logs enter context, and label first-run missing digest state as informational until the first failed command is captured.
