...
fs.writeFileSync(path.join(projectDir, 'agent-feedback.jsonl'), [
  ...
  JSON.stringify({ schema_version: '1', ts: '2026-05-20T00:00:00Z', agent: 'builder_reviewer', signal: 'incorrect', summary: 'Flagged a safe query as unsafe', confidence: 'high', evidence_count: 2 }),
...
const sourceBytes = fs.readFileSync(path.join(projectDir, 'agent-feedback.jsonl'));
...
const checks = [
  ...
  ['counts agents', result.by_agent.builder === 2 && result.by_agent.guardian === 1],
  ['counts quality', result.promotable === 3 && result.corrective === 3 && result.agents.builder.corrective === 1],
  ...
  ['markdown renders advisory rollup', markdown.includes('# Forgeflow Agent Feedback Rollup') && markdown.includes('Advisory only') && markdown.includes('builder') && markdown.includes('## Correction Themes') && markdown.includes('## Promotion Candidates') && markdown.includes('## Staleness') && markdown.includes('Skipped lines: 2') && !markdown.includes('example.internal')],
...
require('assert/strict').deepEqual(fs.readFileSync(path.join(projectDir, 'agent-feedback.jsonl')), sourceBytes);
