...
| `/forgeflow-command-capability` | Render a read-only policy-aware matrix of command-wrapper, Pi, OpenCode, and skill coverage across Forgeflow commands. Required host-parity gaps fail the matrix. |
...
| `/forgeflow-lean-benchmark-runner` | Render an opt-in benchmark runner scaffold with task arms and explicit model-runner commands; default output never calls models or the network. `--run` executes Promptfoo only when `FORGEFLOW_BENCHMARK_ALLOW_NETWORK=1` and a local promptfoo executable is available. |
...
| `/forgeflow-lean-host-cli-probes` | Inspect PATH for optional host CLI executables and print manual lean adapter probe commands without launching host applications. Add `--evidence <json>` to mark manually verified probes. |
...
| `/forgeflow-lean-prime` | Show a single first-run checklist for lean mode, decision evidence, report evidence, telemetry quality, and context-injection eligibility, with one next command to clear the first blocker. Add `--prime-task <text>` to write the lean decision and prime plan artifacts for the current work item. |
