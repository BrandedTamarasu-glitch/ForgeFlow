...
| Learning status | `scripts/forgeflow/show-learning-status.js` | Summarizes project learnings, user profile, agent feedback, review outcomes, next-work outcomes, and first-run results into one advisory health view with fix-first, watch, and healthy lanes. |
...
| Release readiness | `scripts/forgeflow/render-release-readiness.js`, `scripts/forgeflow/render-release-verify.js` | Runs local release readiness checks, release-to-install preflight, optional baseline comparison, optional snapshot writing, optional post-publish verification for local tag/changelog/release-note/source-smoke/update-smoke/installed-runtime-dogfood evidence, compact shareable post-publish summaries, optional post-publish snapshot comparison, and explicit `--github` read-only release/tag evidence. |
