...
- **Lean portability confidence:** `/forgeflow-lean-adapter-contract`, `/forgeflow-lean-hook-contract`, `/forgeflow-lean-rule-canary`, `/forgeflow-lean-robustness`, and `/forgeflow-lean-host-packages` add Ponytail-style confidence checks for adapter targets, hook subprocess behavior, load-bearing rule invariants, known lazy-wrong correctness traps, and host package guidance. They are local and do not install adapters, edit host settings, commit, push, or call the network; `--write` on host packages stores only `.forgeflow/<project>/lean-packages/`.
