...
| First useful win | `scripts/forgeflow/render-first-useful-win.js` | Summarizes early public-safe value evidence from first-run results, pilot evidence, agent feedback, and learning status, then prints the first-use path for install health, profile bootstrap, first task reporting, workflow-ending capture, and shareable summary. |
...
| Telemetry quality | `scripts/forgeflow/render-telemetry-quality.js` | Summarizes local metrics events plus review, agent-feedback, and next-work outcome counts so calibration can distinguish ready evidence from sparse evidence. It is read-only and does not backfill, export, or infer local records. |
...
| First useful win | `scripts/forgeflow/render-first-useful-win.js` | Summarizes aggregate first-run, pilot, agent-feedback, and learning evidence, then prints a first-use path for install health, profile bootstrap, first task reporting, workflow-ending capture, and a shareable useful-win summary. |
