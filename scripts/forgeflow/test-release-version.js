...
const checks = [
  ...
  ['release check runs command wrapper contract test', releaseCheck.includes('node scripts/forgeflow/test-command-wrapper-contract.js')],
  ...
  ['release check runs runtime inventory test', releaseCheck.includes('node scripts/forgeflow/test-runtime-inventory.js')],
  ...
  ['release check runs post-release install verify test', releaseCheck.includes('node scripts/forgeflow/test-render-post-release-install-verify.js')],
  ['release check runs release readiness test', releaseCheck.includes('node scripts/forgeflow/test-render-release-readiness.js')],
  ['release check runs release follow-through test', releaseCheck.includes('node scripts/forgeflow/test-render-release-follow-through.js')],
  ['release check runs release verify test', releaseCheck.includes('node scripts/forgeflow/test-render-release-verify.js')],
  ...
  ['release check runs context wave build test', releaseCheck.includes('node scripts/forgeflow/test-build-context-wave.js')],
  ...
  ['release check runs first task adoption loop test', releaseCheck.includes('node scripts/forgeflow/test-render-first-task-adoption-loop.js')],
  ...
  ['release check runs workflow ending capture test', releaseCheck.includes('node scripts/forgeflow/test-render-workflow-ending-capture.js')],
  ['release check runs command wrapper batch test', releaseCheck.includes('node scripts/forgeflow/test-render-command-wrapper-batch.js')],
  ['release check runs telemetry quality test', releaseCheck.includes('node scripts/forgeflow/test-render-telemetry-quality.js')],
  ...
  ['release check runs update verify test', releaseCheck.includes('node scripts/forgeflow/test-render-update-verify.js')],
  ['release check runs validation and stale artifact plan tests', releaseCheck.includes('node scripts/forgeflow/test-render-validation-plan.js') && releaseCheck.includes('node scripts/forgeflow/test-render-validation-failure-capture.js') && releaseCheck.includes('node scripts/forgeflow/test-render-stale-artifact-plan.js')],
  ['release check runs profile review tests', releaseCheck.includes('node scripts/forgeflow/test-render-profile-bootstrap.js') && releaseCheck.includes('node scripts/forgeflow/test-profile-review.js') && releaseCheck.includes('node scripts/forgeflow/test-profile-compliance.js')],
  ['release check runs next-action contract test', releaseCheck.includes('node scripts/forgeflow/test-next-action-contract.js')],
  ['release check runs output contract test', releaseCheck.includes('node scripts/forgeflow/test-output-contract.js')],
  ['release check runs review-auto classifier test', releaseCheck.includes('node scripts/forgeflow/test-classify-review-auto.js')],
  ['release check runs review evidence schema test', releaseCheck.includes('node scripts/forgeflow/test-check-review-evidence-schema.js')],
  ['release check runs review-auto evidence test', releaseCheck.includes('node scripts/forgeflow/test-render-review-auto-evidence.js')],
  ['release check runs learning signal policy test', releaseCheck.includes('node scripts/forgeflow/test-learning-signal-policy.js')],
  ['release check runs user profile test', releaseCheck.includes('node scripts/forgeflow/test-user-profile.js')],
  ...
  ['release check runs context contract test', releaseCheck.includes('node scripts/forgeflow/test-check-context-contract.js')],
  ...
  ['review command guides Architect topology use', reviewCommand.includes('Architect must use `code_topology_summary` as review-context guidance') && reviewCommand.includes('topology supports prioritization only') && reviewCommand.includes('code_topology_summary.history.trend.status')],
  ['review command guides Product Lead topology use', reviewCommand.includes('use it to prioritize validation around high fan-in/high fan-out files') && reviewCommand.includes('compared code-map trend deltas') && reviewCommand.includes('not proof of runtime behavior')],
  ...
  ['review-auto keeps eligibility matrix conservative', reviewAutoCommand.includes('## Auto-Fix Eligibility Matrix') && reviewAutoCommand.includes('Security, auth, permissions, migrations, dependencies, secrets') && reviewAutoCommand.includes('Surface for human judgment; do not auto-apply')],
  ['review-auto has dry-run classifier command', readText('commands/forgeflow-review-auto-classify.md').includes('classify-review-auto.js') && readme.includes('/forgeflow-review-auto-classify')],
  ['review-auto serializes same-file workers', reviewAutoCommand.includes('### 4b. Partition by target file before dispatch') && reviewAutoCommand.includes('Never let two workers write the same target file from the same preloaded snapshot') && reviewAutoCommand.includes('Workers may run in parallel only when their target files are distinct')],
