...
const missingConflictValue = runRecord([
  '--project-dir',
  projectDir,
  '--category',
  'risk-area',
...
const invalidConflictKey = runRecord([
  '--project-dir',
  projectDir,
  '--category',
  'risk-area',
...
const oversizedConflictValue = runRecord([
  '--project-dir',
  projectDir,
  '--category',
  'risk-area',
...
const checks = [
  ...
  ['cli appends explicit conflict metadata', cliResult.status === 0 && afterCli.includes('"conflict_key":"release_channel"') && afterCli.includes('"conflict_value":"legacy"')],
  ...
  ['incomplete conflict metadata fails', missingConflictValue.status === 1 && missingConflictValue.stderr.includes('conflict_key and conflict_value')],
  ['invalid conflict key fails', invalidConflictKey.status === 1 && invalidConflictKey.stderr.includes('conflict_key')],
  ['oversized conflict value fails', oversizedConflictValue.status === 1 && oversizedConflictValue.stderr.includes('conflict_value')],
