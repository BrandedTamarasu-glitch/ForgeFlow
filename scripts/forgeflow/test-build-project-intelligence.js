...
const checks = [
  ...
  ['includes next work items', Array.isArray(result.next_work_items) && result.next_work_items.length > 0 && result.next_work_items.length <= 5 && result.next_work_items.some((item) => item.title.includes('needs-triage') && item.source === 'readiness' && item.start_with.includes('forgeflow-code-map') && item.validate_with.includes('scripts/forgeflow/build-project-intelligence.js --json') && item.what_to_change.includes('Clear the readiness') && item.how_to_prove.includes('Rebuild project intelligence')) && result.next_work_items.some((item) => item.source === 'agent-feedback' && item.proof_boundary.includes('advisory only') && item.evidence_strength === 'weak' && item.stop_when.includes('stale feedback')) && result.next_work_items.some((item) => item.source === 'review-outcomes' && item.priority === 'high' && item.why.includes('stale-guidance') && item.proof_boundary.includes('aggregate guidance') && item.stop_when.includes('stale guidance')) && result.next_work_items.some((item) => item.source === 'next-work-confidence' && item.evidence_strength === 'weak' && item.proof_boundary.includes('advisory trend evidence')) && result.next_work_items.every((item) => item.proof_boundary && item.what_to_change && item.how_to_prove && item.stop_when && item.confidence && Number.isInteger(item.confidence.score) && ['high', 'medium', 'low'].includes(item.confidence.band) && Array.isArray(item.confidence.reason_codes) && ['strong', 'medium', 'weak'].includes(item.evidence_strength) && Array.isArray(item.start_with) && Array.isArray(item.validate_with))],
