...
const checks = [
  ...
  ['ready dogfood report shareable', ready.dogfood_report.status === 'ready-to-share' && ready.dogfood_report.evidence.completed_steps.includes('release-consumption')],
  ['dogfood report includes downstream efficiency trial', ready.dogfood_report.downstream_trial.some((item) => item.name === 'context-wave-pressure' && item.command.includes('--write-wave-files'))],
  ...
  ['attention loop exposes attention badge', needsSmoke.completion_badge === 'release-consumption-attention' && needsSmoke.dogfood_report.status === 'needs-follow-through'],
  ...
  ['renders markdown', markdown.includes('# Forgeflow Release Consumption Loop') && markdown.includes('## Completion') && markdown.includes('## Dogfood Report') && markdown.includes('Downstream Efficiency Trial') && markdown.includes('downstream-smoke')],
