...
const checks = [
  ...
  ['surfaces candidates', recorded.candidates.some((item) => item.projects.length === 2 && item.occurrences === 3 && item.title.includes('External') && item.source_mix['legacy-learning'] === 3 && item.promotion_candidate.status === 'ready-for-human-review')],
  ['surfaces fallback candidates', recorded.candidates.some((item) => item.projects.length === 2 && item.occurrences === 3 && item.title.includes('Rollback') && item.source_mix['project-learning-candidate'] === 3 && item.promotion_candidate.boundary.includes('manual'))],
  ...
  ['renders markdown', markdown.includes('# Forgeflow Learnings - all') && markdown.includes('Candidates for promotion') && markdown.includes('Sources: project candidates: 3') && markdown.includes('Promotion: ready-for-human-review') && markdown.includes('do not auto-promote') && markdown.includes('project-learning-candidate')],
