...
const checks = [
  ...
  ['review wave automation present', result.review_wave_automation.status === 'recommended' && result.review_wave_automation.next === '/forgeflow-context-wave-plan' && result.review_wave_automation.write_command === '/forgeflow-context-wave-plan --write-wave-files' && result.review_wave_automation.build_command === '/forgeflow-context-wave-build'],
  ['review wave automation separates mutating commands', result.review_wave_automation.commands[0].mutates === false && result.review_wave_automation.commands.some((item) => item.step === 'write-wave-files' && item.mutates === true)],
  ['review wave automation renders', markdown.includes('## Review Wave Automation') && markdown.includes('preview-wave-plan: /forgeflow-context-wave-plan') && markdown.includes('Mutates: no')],
  ['clean next actions stable', smallLowSavings.review_wave_suggested === false && cleanMarkdown.includes('Review wave suggested: no')],
  ['clean review wave automation stable', smallLowSavings.review_wave_automation.status === 'not-needed'],
