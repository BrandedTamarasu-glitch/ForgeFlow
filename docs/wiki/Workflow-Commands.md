...
| `/forgeflow-lean-mode` | Show or persist the project-local lean guidance profile. Use `--profile off|lite|balanced|strict|ultra --write` to save `.forgeflow/<project>/context/lean-policy.md` and `.json`. The mode is advisory and only affects whether/how compact lean guidance is injected into context packs. |
| `/forgeflow-lean-portability` | Generate or check portable lean rule copies for generic agent hosts, Cursor, Windsurf, Copilot, and skill-style adapters. Add `--write` to save `.forgeflow/<project>/lean-portability/`. |
...
| `/forgeflow-lean-session` | Render compact always-on lean session guidance plus `LEAN:<profile>` statusline text for hook or adapter experiments. Display-only; it does not install hooks or edit settings. |
