...
| Learning status | `scripts/forgeflow/show-learning-status.js` | Summarizes project learnings, user profile, agent feedback, review outcomes, next-work outcomes, and first-run results into one advisory health view with fix-first, watch, healthy lanes, and signal-quality scoring. |
| Project health timeline | `scripts/forgeflow/show-project-health-timeline.js` | Renders a read-only local timeline from code-map history, context-advisor history, latest-insights readiness, and learning-signal quality. |
