...
Implementation status: **Phases 0, 1 and 2, plus F3.1 and F3.2 complete**. Contracts, bounded selection, managed-host entry points, corrected atomicity guidance and compatible skill evaluation are implemented. Change propagation, visual acceptance, persistence recovery, review calibration, provider compatibility and web release qualification are implemented in the evaluation cohort; normal automatic execution awaits qualification. The native release branch and other three procedures remain planned.

**Next item: F3.3 — Native release qualification.** Exercise installed identity and first-run/restart/update/uninstall/rollback behavior in disposable profiles on an available native environment. F3.2 now distinguishes intended bytes from served artifacts and separates identity from interaction outcomes. All six implemented capabilities remain in evaluation; no public release or model benefit is claimed.
...
**Session checkpoint:** implementation is complete through F3.2; F3.3 has not started. See [current status and resumption steps](STATUS.md). Phase 1 was published through `cebf945`; the packaged release version is unchanged.
