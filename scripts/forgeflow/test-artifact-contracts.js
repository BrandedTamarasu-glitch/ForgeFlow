...
async function main() {
  ...
  const checks = [
    ...
    ['project intelligence user profile contract', hasKeys(intelligence.user_profile, ['status', 'injected', 'issue_count', 'suggestion_count', 'conflict_count', 'records', 'files', 'guidance']) && hasKeys(intelligence.user_profile.records, ['global', 'project', 'active'])],
    ['project intelligence next work contract', Array.isArray(intelligence.next_work_items) && intelligence.next_work_items.every((item) => hasKeys(item, ['title', 'priority', 'source', 'why', 'start_with', 'validate_with', 'proof_boundary', 'what_to_change', 'how_to_prove', 'stop_when', 'evidence_strength', 'confidence']) && Array.isArray(item.start_with) && Array.isArray(item.validate_with) && ['strong', 'medium', 'weak'].includes(item.evidence_strength) && hasKeys(item.confidence, ['score', 'band', 'reason_codes', 'boundary']) && Array.isArray(item.confidence.reason_codes))],
    ['project intelligence next work confidence contract', hasKeys(intelligence.next_work_confidence, ['status', 'file', 'records', 'invalid_lines', 'by_outcome', 'by_source', 'confidence_calibration', 'recommendation'])],
