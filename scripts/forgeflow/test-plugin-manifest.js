...
const piPackage = JSON.parse(fs.readFileSync(path.join(repoRoot, 'pi-extension', 'package.json'), 'utf8'));
...
const checks = [
  ...
  ['claude plugin advertises skills', plugin.components?.skills === 'skills/'],
  ...
  ['copilot plugin advertises skills', copilotPlugin.skills === 'skills/'],
  ['copilot plugin advertises hooks', copilotPlugin.hooks === 'hooks/copilot-hooks.json'],
  ['copilot hook manifest present', fs.readFileSync(path.join(repoRoot, 'hooks', 'copilot-hooks.json'), 'utf8').includes('powershell')],
  ['gemini extension manifest present', geminiExtension.name === 'Forgeflow' && geminiExtension.contextFileName === 'AGENTS.md'],
  ['pi extension package present', piPackage.name === 'forgeflow-pi-extension' && piPackage.main === 'index.js'],
