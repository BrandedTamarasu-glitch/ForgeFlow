...
- **Evidence readiness:** benchmark evidence grading, host CLI proof states, failure-digest aftercare, RTK command-policy alignment, Lean Prime bootstrap guidance, and release-readiness advisory warnings before users make performance, portability, or host-verification claims.
...
- **Command interface evidence:** `/forgeflow-command-interface-evidence --input <sanitized-observations.json>` reviews only explicitly supplied, bounded command-chain observations. It reports aggregate repetition and outcome evidence, does not discover command history or read raw diagnostics, and never creates or promotes a wrapper. A local aggregate report requires both `--write-report` and `--project-dir <path>`; it is confined to that session directory. Candidate results remain human-review inputs and do not claim savings until a future paired comparison exists.
- **Command interface learning:** `/forgeflow-command-interface-learning --input <sanitized-observations.json>` turns a qualifying aggregate repeat pattern into a local advisory-memory preview. Suggestions are enabled by default and can be suppressed per project; durable project memory always requires the exact candidate ID and `--write`.
