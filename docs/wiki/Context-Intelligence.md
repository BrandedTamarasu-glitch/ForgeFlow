...
| Review-auto classifier and sandbox proposal | `scripts/forgeflow/check-review-evidence-schema.js`, `scripts/forgeflow/classify-review-auto.js`, `scripts/forgeflow/render-review-auto-evidence.js`, `scripts/forgeflow/run-review-autofix-sandbox.js` | Validates captured findings shape, path hazards, multi-file findings, and obvious safety hazards, previews safe, risky, and blocker buckets from captured review findings JSON, writes a local evidence artifact, and can run an explicit deterministic proposal in an isolated temp sandbox. The Phase 4 policy contract makes safe mean future sandbox proposal eligibility only; Phase 5 emits `proposal.json`, `proposal.md`, and `proposal.diff` under `.forgeflow/<project>/review-auto/proposals/` without mutating the source checkout. |
