...
- **Runtime drift preview:** `/forgeflow-runtime-drift --preview-repair` shows the exact managed helpers that `/update-forgeflow --repair` would install, replace, chmod, or leave alone. The preview is read-only and does not mutate installed files.
- **Project health timeline:** `/forgeflow-health-timeline` summarizes local code-map history, context-advisor history, latest-insights readiness, and learning-signal quality into a compact advisory timeline.
