...
| Release readiness | `scripts/forgeflow/render-release-readiness.js` | Runs the local release-check command list, groups blockers by readiness area, and never tags, pushes, publishes, or calls GitHub. |
...
| Agent feedback rollup | `scripts/forgeflow/rollup-agent-feedback.js` | Summarizes local `agent-feedback.jsonl` by reviewer, signal, promotable count, corrective count, skipped invalid/private lines, and filtered advisory examples. |
